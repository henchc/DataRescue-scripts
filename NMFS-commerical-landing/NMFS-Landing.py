...
r = requests.get('https://www.st.nmfs.noaa.gov/pls/webpls/FT_HELP.SPECIES', params=payload)
...
for sp in species:
    ...
    if not os.path.exists(sp.replace(",", "").replace(" ", "-").replace("/", "_")): # if need to restart script
        ...
        soup = BeautifulSoup(r.content, "lxml")
        states = [x.text for x in soup.find("select", {"name": "qstate"}).findAll("option")]
        ...
        for st in states:
            ...
            payload = {'qspecies': sp,
                       ...
                       'qstate': st,
                       'qoutput_type': 'TABLE'}
            r = requests.get('http://www.st.nmfs.noaa.gov/pls/webpls/MF_MONTHLY_LANDINGS.RESULTS', params=payload)
...
for st in states:
    ...
    r = requests.get('https://www.st.nmfs.noaa.gov/pls/webpls/MF_MONTHLY_LANDINGS.RESULTS', params=payload)
